...
# Local imports
from r2x.config_models import ReEDSConfig, PypsaConfig
...
class PypsaExporter(BaseExporter):
    """PyPSA exporter class.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.output_data = {}
        if not isinstance(self.config.output_config, PypsaConfig):
            msg = (
                f"Output config is of type {type(self.config.output_config)}. "
                "It should be type of `PypsaConfig`."
            )
            raise TypeError(msg)
        ...
        self.output_config = self.config.input_config.to_class(PypsaConfig, self.config.output_config)
        self.property_map = self.output_config.defaults.get("pypsa_property_map", {})
        self.unit_map = self.output_config.defaults.get("pypsa_unit_map", {})
        self.output_fields = self.output_config.defaults["table_data"]
        self.year = get_year(self.output_config)
        assert self.year is not None

    def run(self, *args, path=None, **kwargs) -> "PypsaExporter":
        """Run PyPSA exporter workflow.
        ...
        for reserve in reserves:
            ...
            if reserve["name"] in reserve_map:
                ...
                output_dict["contributing_devices"] = str(tuple(contributing_devices)).replace(  # type: ignore
                    "'", ""
                )  # PyPSA expects a tuple for this field that looks like "(val1,val2,val3)"
    ...
    def export_data(self) -> None:
        ...
        logger.debug("Saving PyPSA data and timeseries files.")
...
def apply_operation_table_data(
    ...
    if "fuel_cost" in variable.keys():
        ...
        # done by PyPSA when it parses .csv files
