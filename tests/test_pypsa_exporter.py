...
from r2x.exporter.pypsa import PypsaExporter, apply_operation_table_data, get_psy_fields
...
@pytest.fixture
def pypsa_exporter(scenario_instance, infrasys_test_system, tmp_folder):
    return PypsaExporter(config=scenario_instance, system=infrasys_test_system, output_folder=tmp_folder)


@pytest.mark.pypsa
def test_pypsa_exporter_instance(pypsa_exporter):
    assert isinstance(pypsa_exporter, PypsaExporter)


@pytest.mark.pypsa
def test_pypsa_exporter_run(pypsa_exporter, tmp_folder):
    exporter = pypsa_exporter.run()
...
def test_pypsa_exporter_empty_storage(caplog, pypsa_exporter):
    pypsa_exporter.process_storage_data()
